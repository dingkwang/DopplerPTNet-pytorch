...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        ...
        # Initialize with clones to avoid modifying original tensors
        filled_xyz = xyz.clone()
        filled_new_xyz = new_xyz.clone()

        # Handle case where last dimension isn't 3
        if xyz.shape[-1] != 3:
            filled_xyz = torch.cat([
                xyz.clone(),  # Clone to ensure original isn't modified
                torch.zeros(xyz.shape[0], 3 - xyz.shape[-1], device=xyz.device, dtype=xyz.dtype)
            ], dim=1)

        if new_xyz.shape[-1] != 3:
            filled_new_xyz = torch.cat([
                new_xyz.clone(),  # Clone to ensure original isn't modified
                torch.zeros(new_xyz.shape[0], 3 - new_xyz.shape[-1], device=new_xyz.device, dtype=new_xyz.dtype)
            ], dim=1)

        assert filled_xyz.is_contiguous() and filled_new_xyz.is_contiguous()
        assert filled_xyz.shape[1] == 3 and filled_new_xyz.shape[1] == 3

        m = filled_new_xyz.shape[0]
        idx = torch.cuda.IntTensor(m, nsample).zero_()
        dist2 = torch.cuda.FloatTensor(m, nsample).zero_()

        pointops_cuda.knnquery_cuda(m, nsample, filled_xyz, filled_new_xyz, 
                                  offset, new_offset, idx, dist2)
        
        assert torch.all((idx >= 0) & (idx < filled_xyz.shape[0])), \
            f"Index out of range in 'idx'. Expected range [0, {filled_xyz.shape[0]}), got idx with min {idx.min()} and max {idx.max()}"
...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, feature_only=True):
    ...
    c_xyz = xyz.shape[1]
    grouped_xyz = xyz[idx.view(-1).long(), :c_xyz].view(m, nsample, c_xyz) # (m, nsample, 3)
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    # Ensure idx is within valid range for feat
    assert torch.all((idx >= 0) & (idx < feat.shape[0])), \
        f"Index out of range in 'idx'. Expected range [0, {feat.shape[0]}), got idx with min {idx.min()} and max {idx.max()}"
